...
file(GLOB APUI_UTILS_FILES "${CMAKE_CURRENT_LIST_DIR}/Aperture/*.cmake")
...
# automatically include all files in the Aperture subfolder
foreach(UTILS_FILE ${UTILS_FILES})
	include("${UTILS_FILE}")
endforeach()
# #####################################
...
macro(ns_pull_config_vars)
	...
	get_property(NS_CONFIG_VULKAN_VALIDATIONLAYERS_VERSION GLOBAL PROPERTY NS_CONFIG_VULKAN_VALIDATIONLAYERS_VERSION)
	get_property(NS_CONFIG_VULKAN_VALIDATIONLAYERS_ANDROID_URL GLOBAL PROPERTY NS_CONFIG_VULKAN_VALIDATIONLAYERS_ANDROID_URL)
endmacro()
...
macro(ns_pull_output_vars LIB_OUTPUT_DIR DLL_OUTPUT_DIR)
	...
	set(PRE_PATH "${NS_CMAKE_PLATFORM_PREFIX}${NS_OUTPUT_DIRECTORY_PLATFORM_POSTFIX}${NS_CMAKE_GENERATOR_PREFIX}${NS_CMAKE_COMPILER_POSTFIX}")
...
function(ns_set_project_ide_folder TARGET_NAME PROJECT_SOURCE_DIR)
	...
	if(NS_SUBMODULE_MODE)
		set_property(TARGET ${TARGET_NAME} PROPERTY FOLDER "${NS_SUBMODULE_ROOT_IDE_FOLDER}/${IDE_FOLDER}")
	else()
...
# #####################################
function(ns_make_windowapp TARGET_NAME)
	set_property(TARGET ${TARGET_NAME} PROPERTY WIN32_EXECUTABLE ON)
	target_compile_definitions(${TARGET_NAME} PRIVATE NS_WINDOWAPP=1)

	if (COMMAND ns_platformhook_make_windowapp)
		ns_platformhook_make_windowapp(${TARGET_NAME})
	endif()	
endfunction()
...
# #####################################
# ## ns_create_link(<source> <destination-folder> <destination-name>)
# #####################################
function(ns_create_link SOURCE DEST_FOLDER DEST_NAME)
...
# #####################################
# ## ns_package_files(TARGET_NAME SRC_FOLDER DST_FOLDER)
# #####################################
#
...
# Internally it just forwards to ns_platformhook_package_files.
# #####################################
function(ns_package_files TARGET_NAME SRC_FOLDER DST_FOLDER)

	if (COMMAND ns_platformhook_package_files)
		ns_platformhook_package_files(${TARGET_NAME} ${SRC_FOLDER} ${DST_FOLDER})
	endif()
...
# #####################################
# ## ns_copy_plugin_bundle(TARGET_NAME FILE_NAME)
# #####################################
#
# Copies the given file with the .nsPluginBundle extension from the current source directory into the target directory.
#
# #####################################
function(ns_copy_plugin_bundle TARGET_NAME FILE_NAME)

	add_custom_command(TARGET ${TARGET_NAME} POST_BUILD
			COMMAND ${CMAKE_COMMAND} -E copy_if_different "${CMAKE_CURRENT_SOURCE_DIR}/${FILE_NAME}.nsPluginBundle" $<TARGET_FILE_DIR:${TARGET_NAME}>
