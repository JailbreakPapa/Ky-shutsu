...
-unpack <paths>
    One or multiple paths to wpck files that shall be extracted.

    Example:
      -unpack "path/to/file.wpck" "another/file.wpck"
...
Examples:
    wpckTool.exe "C:/Stuff"
      Packs all data in "C:/Stuff" into "C:/Stuff.wpck"

    wpckTool.exe "C:/Stuff" -out "C:/MyStuff.wpck"
      Packs all data in "C:/Stuff" into "C:/MyStuff.wpck"

    wpckTool.exe "C:/Stuff.wpck"
      Unpacks all data from the archive into "C:/Stuff"

    wpckTool.exe "C:/Stuff.wpck" -out "C:/MyStuff"
...
class wpckBuilderImpl : public wpckBuilder
{
protected:
...
class wpckReaderImpl : public wpckReader
{
public:
protected:
...
class wpckTool : public nsApplication
{
public:
  ...
  static wpckBuilder::InclusionMode PackFileCallback(nsStringView sFile)
  {
    ...
    if (ext.IsEqual_NoCase("jpg") || ext.IsEqual_NoCase("jpeg") || ext.IsEqual_NoCase("png"))
      return wpckBuilder::InclusionMode::Uncompressed;
    ...
    if (ext.IsEqual_NoCase("dds"))
      return wpckBuilder::InclusionMode::Compress_zstd_fast;

    return wpckBuilder::InclusionMode::Compress_zstd_average;
  }

  nsResult Pack()
  {
    wpckBuilderImpl archive;

    for (const auto& folder : m_sInputs)
    {
      archive.AddFolder(folder, wpckCompressionMode::Compressed_zstd, PackFileCallback);
    }

    if (m_sOutput.IsEmpty())
    {
      nsStringBuilder sArchive = m_sInputs[0];
      sArchive.Append(".wpck");
    ...
    {
      nsLog::Error("Failed to write the wpck");
  ...
  nsResult Unpack()
  {
    for (const auto& file : m_sInputs)
    {
      ...
      // we assume that the user only gives us files that are wpcks
      if (!wpckUtils::IsAcceptedArchiveFileExtensions(nsPathUtils::GetFileExtension(file)))
      {
        wpckUtils::GetAcceptedArchiveFileExtensions().PushBack(nsPathUtils::GetFileExtension(file));
      }

      wpckReaderImpl reader;
...
NS_APPLICATION_ENTRY_POINT(wpckTool);
