...
nsResult nsImageView::SaveTo(nsStringView sFileName) const
{
  ...
  if (const nsImageFileFormat* pFormat = nsImageFileFormat::GetWriterFormat(it.GetStartPointer()))
  {
...
{
  NS_IGNORE_UNUSED(uiMipLevel);
  NS_IGNORE_UNUSED(uiFace);
  NS_IGNORE_UNUSED(uiArrayIndex);
  NS_IGNORE_UNUSED(uiPlaneIndex);
...
nsResult nsImage::LoadFrom(nsStringView sFileName)
{
  ...
  if (const nsImageFileFormat* pFormat = nsImageFileFormat::GetReaderFormat(it.GetStartPointer()))
  {
