...
// Identify the compiler and declare the WDSC_COMPILER_xxxx defines
#include <WDBase/config/wdcorpcompiler.h>

// Identify traits which this compiler supports, or does not support
#include <WDBase/config/wdcorpcompilertraits.h>

// Identify the platform and declare the WDSC_xxxx defines
#include <WDBase/config/wdcorpplatform.h>
...
#include <WDBase/version.h>

// Define common SI unit macros
#include <WDBase/wdcorpunits.h>
...
#if defined(WDSC_COMPILER_GNUC) || defined(WDSC_COMPILER_MSVC) || defined(WDSC_WCHAR_T_NON_NATIVE) || defined(WDSC_PLATFORM_SONY)
...
// is defined.
#if defined(WDSC_COMPILER_CLANG) && defined(WDSC_PLATFORM_APPLE)
	#include <stddef.h>
#endif
...
// would have put this into the compilertraits header.
#if !defined(WDSC_COMPILER_HAS_INTTYPES) && (!defined(_MSC_VER) || (_MSC_VER > 1500)) && (defined(WDSC_COMPILER_IS_C99) || defined(INT8_MIN) || defined(WDSC_COMPILER_HAS_C99_TYPES) || defined(_SN_STDINT_H))
	#define WDSC_COMPILER_HAS_INTTYPES
#endif

#ifdef WDSC_COMPILER_HAS_INTTYPES // If the compiler supports inttypes...
	// ------------------------------------------------------------------------
	...
	#if !defined(WDSC_COMPILER_MSVC) || (defined(WDSC_COMPILER_MSVC) && WDSC_COMPILER_VERSION >= 1800)
	...
	#if defined(__MINGW32__) || (defined(WDSC_PLATFORM_ANDROID) && !(defined(WDSC_ANDROID_SDK_LEVEL) && WDSC_ANDROID_SDK_LEVEL >= 21))
   ...
   #if defined(WDSC_PLATFORM_SUN) || defined(WDSC_PLATFORM_SGI)
	   #if (WDSC_PLATFORM_PTR_SIZE == 4)
   ...
   #elif defined(WDSC_COMPILER_MSVC) 
...
// type sizes
...
#if defined(WDSC_COMPILER_HAS_INTTYPES) && (!defined(WDSC_COMPILER_MSVC) || (defined(WDSC_COMPILER_MSVC) && WDSC_COMPILER_VERSION >= 1800))
	#define WDSC_COMPILER_HAS_C99_FORMAT_MACROS 
#endif

#ifndef WDSC_COMPILER_HAS_C99_FORMAT_MACROS 
	// ------------------------------------------------------------------------
	...
	#if defined(WDSC_COMPILER_MSVC) // VC++ 7.1+ understands long long as a data type but doesn't accept %ll as a printf specifier.
		#define WDSC_PRI_64_LENGTH_SPECIFIER "I64"
		#define WDSC_SCN_64_LENGTH_SPECIFIER "I64"
	#else
		#define WDSC_PRI_64_LENGTH_SPECIFIER "ll"
		#define WDSC_SCN_64_LENGTH_SPECIFIER "ll"
	...
	#if defined(WDSC_COMPILER_IS_C99) || defined(WDSC_COMPILER_GNUC)
		#define PRId8     "hhd"
		#define PRIi8     "hhi"
		#define PRIo8     "hho"
		#define PRIu8     "hhu"
	...
	#define PRId64        WDSC_PRI_64_LENGTH_SPECIFIER "d"
	#define PRIi64        WDSC_PRI_64_LENGTH_SPECIFIER "i"
	#define PRIo64        WDSC_PRI_64_LENGTH_SPECIFIER "o"
	#define PRIu64        WDSC_PRI_64_LENGTH_SPECIFIER "u"
	#define PRIx64        WDSC_PRI_64_LENGTH_SPECIFIER "x"
	#define PRIX64        WDSC_PRI_64_LENGTH_SPECIFIER "X"
	...
	#define SCNd64        WDSC_SCN_64_LENGTH_SPECIFIER "d"
	#define SCNi64        WDSC_SCN_64_LENGTH_SPECIFIER "i"
	#define SCNo64        WDSC_SCN_64_LENGTH_SPECIFIER "o"
	#define SCNu64        WDSC_SCN_64_LENGTH_SPECIFIER "u"
	#define SCNx64        WDSC_SCN_64_LENGTH_SPECIFIER "x"

	#if defined(WDSC_COMPILER_MSVC) && (WDSC_COMPILER_VERSION >= 1900)
		#define SCNdPTR       PRIdPTR
		#define SCNiPTR       PRIiPTR
		#define SCNoPTR       PRIoPTR
		#define SCNuPTR       PRIuPTR
		#define SCNxPTR       PRIxPTR
	#elif (WDSC_PLATFORM_PTR_SIZE == 4)
...
// 
	...
	#if defined(WDSC_COMPILER_MSVC) || (defined(WDSC_COMPILER_INTEL) && defined(WDSC_PLATFORM_WINDOWS))
		#if defined(__cplusplus)
			typedef bool bool8_t;
		#else
			typedef int8_t bool8_t;
		#endif
	#else // WDSC_COMPILER_GNUC generally uses 4 bytes per bool.
		typedef int8_t bool8_t;
	#endif
#endif
...
//
#if !defined(_INTPTR_T_DEFINED) && !defined(_intptr_t_defined) && !defined(WDSC_COMPILER_HAS_C99_TYPES)
	#if (WDSC_PLATFORM_PTR_SIZE == 4)
...
#if !defined(_UINTPTR_T_DEFINED) && !defined(_uintptr_t_defined) && !defined(WDSC_COMPILER_HAS_C99_TYPES)
	#if (WDSC_PLATFORM_PTR_SIZE == 4)
...
#if !defined(WDSC_COMPILER_HAS_INTTYPES)
...
//
	...
	// so for all compilers, as size_t may be based on int, long, or long long.
		...
		#if defined(_MSC_VER) && (WDSC_PLATFORM_PTR_SIZE == 8)
...
// _CHAR16T and let EABase define char16_t and WDSC_CHAR16.
//
...
//
// The WDSC_WCHAR_UNIQUE symbol is defined to 1 if wchar_t is distinct from
...
//
// The WDSC_CHAR8_UNIQUE symbol is defined to 1 if char8_t is distinct type 
// from char in the type system, and defined to 0 if otherwise.

#if !defined(WDSC_CHAR16_NATIVE)
	// To do: Change this to be based on WDSC_COMPILER_NO_NEW_CHARACTER_TYPES.
	#if defined(_MSC_VER) && (_MSC_VER >= 1600) && defined(_HAS_CHAR16_T_LANGUAGE_SUPPORT) && _HAS_CHAR16_T_LANGUAGE_SUPPORT // VS2010+
		#define WDSC_CHAR16_NATIVE 1
	#elif defined(WDSC_COMPILER_CLANG) && defined(WDSC_COMPILER_CPP11_ENABLED)
		#if __has_feature(cxx_unicode_literals)
			#define WDSC_CHAR16_NATIVE 1
		#elif (WDSC_COMPILER_VERSION >= 300) && !(defined(WDSC_PLATFORM_IPHONE) || defined(WDSC_PLATFORM_OSX))
			#define WDSC_CHAR16_NATIVE 1
		#elif defined(WDSC_PLATFORM_APPLE)
			#define WDSC_CHAR16_NATIVE 1 
		#else
			#define WDSC_CHAR16_NATIVE 0
		#endif
	#elif defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 404) && defined(__CHAR16_TYPE__) && defined(WDSC_COMPILER_CPP11_ENABLED)// EDG 4.4+.
		#define WDSC_CHAR16_NATIVE 1
	#elif defined(WDSC_COMPILER_GNUC) && (WDSC_COMPILER_VERSION >= 4004) && !defined(WDSC_COMPILER_EDG) && (defined(WDSC_COMPILER_CPP11_ENABLED) || defined(__STDC_VERSION__)) // g++ (C++ compiler) 4.4+ with -std=c++0x or gcc (C compiler) 4.4+ with -std=gnu99
		#define WDSC_CHAR16_NATIVE 1
	#elif defined(WDSC_COMPILER_SN) && (__EDG_VERSION__ >=  408) && defined(WDSC_COMPILER_CPP11_ENABLED)
		#define WDSC_CHAR16_NATIVE 1
	#else
		#define WDSC_CHAR16_NATIVE 0
	#endif
...
#if !defined(WDSC_CHAR32_NATIVE)                    // Microsoft currently ties char32_t language support to char16_t language support. So we use CHAR16_T here.
	// To do: Change this to be based on WDSC_COMPILER_NO_NEW_CHARACTER_TYPES.
	#if defined(_MSC_VER) && (_MSC_VER >= 1600) && defined(_HAS_CHAR16_T_LANGUAGE_SUPPORT) && _HAS_CHAR16_T_LANGUAGE_SUPPORT // VS2010+
		#define WDSC_CHAR32_NATIVE 1
	#elif defined(WDSC_COMPILER_CLANG) && defined(WDSC_COMPILER_CPP11_ENABLED)
		#if __has_feature(cxx_unicode_literals)
			#define WDSC_CHAR32_NATIVE 1
		#elif (WDSC_COMPILER_VERSION >= 300) && !(defined(WDSC_PLATFORM_IPHONE) || defined(WDSC_PLATFORM_OSX))
			...
			#define WDSC_CHAR32_NATIVE 1 
		#else
			#define WDSC_CHAR32_NATIVE 0
		#endif
	#elif defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 404) && defined(__CHAR32_TYPE__) && defined(WDSC_COMPILER_CPP11_ENABLED)// EDG 4.4+.
		#define WDSC_CHAR32_NATIVE 1
	#elif defined(WDSC_COMPILER_GNUC) && (WDSC_COMPILER_VERSION >= 4004) && !defined(WDSC_COMPILER_EDG) && (defined(WDSC_COMPILER_CPP11_ENABLED) || defined(__STDC_VERSION__)) // g++ (C++ compiler) 4.4+ with -std=c++0x or gcc (C compiler) 4.4+ with -std=gnu99
		#define WDSC_CHAR32_NATIVE 1
#elif defined(WDSC_COMPILER_SN) && (__EDG_VERSION__ >=  408) && defined(WDSC_COMPILER_CPP11_ENABLED)
...
#if WDSC_CHAR16_NATIVE || WDSC_CHAR32_NATIVE
	#define WDSC_WCHAR_UNIQUE 1
#else
	#define WDSC_WCHAR_UNIQUE 0
#endif
...
//
#define WDSC_LIMITS_DIGITS_S(T)  ((sizeof(T) * 8) - 1)
#define WDSC_LIMITS_DIGITS_U(T)  ((sizeof(T) * 8))
#define WDSC_LIMITS_DIGITS(T)    ((WDSC_LIMITS_IS_SIGNED(T) ? WDSC_LIMITS_DIGITS_S(T) : WDSC_LIMITS_DIGITS_U(T)))
#define WDSC_LIMITS_IS_SIGNED(T) ((T)(-1) < 0)
#define WDSC_LIMITS_MIN_S(T)     ((T)((T)1 << WDSC_LIMITS_DIGITS_S(T)))
#define WDSC_LIMITS_MIN_U(T)     ((T)0)
#define WDSC_LIMITS_MIN(T)       ((WDSC_LIMITS_IS_SIGNED(T) ? WDSC_LIMITS_MIN_S(T) : WDSC_LIMITS_MIN_U(T)))
#define WDSC_LIMITS_MAX_S(T)     ((T)(((((T)1 << (WDSC_LIMITS_DIGITS(T) - 1)) - 1) << 1) + 1))
#define WDSC_LIMITS_MAX_U(T)     ((T)~(T)0)
#define WDSC_LIMITS_MAX(T)       ((WDSC_LIMITS_IS_SIGNED(T) ? WDSC_LIMITS_MAX_S(T) : WDSC_LIMITS_MAX_U(T)))

#if !defined(CHAR8_MIN)
	#define CHAR8_MIN WDSC_LIMITS_MIN(char8_t)
#endif

#if !defined(CHAR8_MAX)
	#define CHAR8_MAX WDSC_LIMITS_MAX(char8_t)
#endif

#if !defined(CHAR16_MIN)
	#define CHAR16_MIN WDSC_LIMITS_MIN(char16_t)
#endif

#if !defined(CHAR16_MAX)
	#define CHAR16_MAX WDSC_LIMITS_MAX(char16_t)
#endif

#if !defined(CHAR32_MIN)
	#define CHAR32_MIN WDSC_LIMITS_MIN(char32_t)
#endif

#if !defined(CHAR32_MAX)
	#define CHAR32_MAX WDSC_LIMITS_MAX(char32_t)
#endif



// WDSC_CHAR8 / WDSC_CHAR16 / WDSC_CHAR32 / WDSC_WCHAR 
//
// Supports usage of portable string constants.
//
// Example usage:
//     const char16_t* str = WDSC_CHAR16("Hello world");
//     const char32_t* str = WDSC_CHAR32("Hello world");
//     const char16_t  c   = WDSC_CHAR16('\x3001');
//     const char32_t  c   = WDSC_CHAR32('\x3001');
//
#ifndef WDSC_CHAR8
	#if WDSC_CHAR8_UNIQUE
		#define WDSC_CHAR8(s) u8 ## s
	#else
		#define WDSC_CHAR8(s) s
	#endif
#endif

#ifndef WDSC_WCHAR
	#define WDSC_WCHAR_(s) L ## s
	#define WDSC_WCHAR(s)  WDSC_WCHAR_(s)
#endif

#ifndef WDSC_CHAR16
	#if WDSC_CHAR16_NATIVE && !defined(_MSC_VER) // Microsoft doesn't support char16_t string literals.
		#define WDSC_CHAR16_(s) u ## s
		#define WDSC_CHAR16(s)  WDSC_CHAR16_(s)
	#elif (WDSC_WCHAR_SIZE == 2)
		#if defined(_MSC_VER) && (_MSC_VER >= 1900) && defined(__cplusplus) // VS2015 supports u"" string literals.
			#define WDSC_CHAR16_(s) u ## s
			#define WDSC_CHAR16(s)  WDSC_CHAR16_(s)
		#else
			#define WDSC_CHAR16_(s) L ## s
			#define WDSC_CHAR16(s)  WDSC_CHAR16_(s)
		#endif
	#else
		//#define WDSC_CHAR16(s) // Impossible to implement efficiently.
	#endif
#endif

#ifndef WDSC_CHAR32
	#if WDSC_CHAR32_NATIVE && !defined(_MSC_VER) // Microsoft doesn't support char32_t string literals.
		#define WDSC_CHAR32_(s) U ## s
		#define WDSC_CHAR32(s)  WDSC_CHAR32_(s)
	#elif (WDSC_WCHAR_SIZE == 2)
		#if defined(_MSC_VER) && (_MSC_VER >= 1900) && defined(__cplusplus) // VS2015 supports u"" string literals.
			#define WDSC_CHAR32_(s) U ## s
			#define WDSC_CHAR32(s)  WDSC_CHAR32_(s)
		#else
			//#define WDSC_CHAR32(s) // Impossible to implement.
		#endif
	#elif (WDSC_WCHAR_SIZE == 4)
		#define WDSC_CHAR32_(s) L ## s
		#define WDSC_CHAR32(s)  WDSC_CHAR32_(s)
	#else
...
//
#if defined(WDBASE_ENABLE_EATEXT_MACROS)
	#define EAText8(x)   x
	#define EAChar8(x)   x

	#define EAText16(x)  WDSC_CHAR16(x)
	#define EAChar16(x)  WDSC_CHAR16(x)
#endif
...
//
#if defined(WDSC_COMPILER_NO_CONSTEXPR)
	#ifndef EAArrayCount
...
//
...
#elif !defined(__cplusplus) && defined(WDSC_PLATFORM_ANDROID) && ((defined(__STDC_VERSION__) && __STDC_VERSION__ < 201100L) || !defined(__STDC_VERSION__))
...
#elif defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 401) && defined(WDSC_COMPILER_CPP11_ENABLED)
	// static_assert is defined by the compiler.
	...
	#define NEED_CUSTOM_STATIC_ASSERT
		...
		#define WDSC_STATIC_ASSERT_UNUSED_ATTRIBUTE        __attribute__((unused))
	#else
		#define WDSC_STATIC_ASSERT_UNUSED_ATTRIBUTE
	#endif
	#define WDSC_STATIC_ASSERT_TOKEN_PASTE(a,b)        a ## b
	#define WDSC_STATIC_ASSERT_CONCATENATE_HELPER(a,b) WDSC_STATIC_ASSERT_TOKEN_PASTE(a,b)

	#if defined(__COUNTER__) // If this extension is available, which allows multiple statements per line...
		#define static_assert(expression, description) typedef char WDSC_STATIC_ASSERT_CONCATENATE_HELPER(compileTimeAssert,__COUNTER__) [((expression) != 0) ? 1 : -1] WDSC_STATIC_ASSERT_UNUSED_ATTRIBUTE
	#else
		#define static_assert(expression, description) typedef char WDSC_STATIC_ASSERT_CONCATENATE_HELPER(compileTimeAssert,__LINE__) [((expression) != 0) ? 1 : -1] WDSC_STATIC_ASSERT_UNUSED_ATTRIBUTE
	#endif
...
// WDSC_IS_ENABLED
//
// WDSC_IS_ENABLED is intended to be used for detecting if compile time features are enabled or disabled.
//
...
// must be set to either WDSC_ENABLED or WDSC_DISABLED.  (Do not try to set the feature define directly to some other
// value.)
//
...
//      // WDSC_ENABLED or WDSC_DISABLED.
//      #define USER_PROVIDED_FEATURE_DEFINE WDSC_ENABLED
//
//      #if WDSC_IS_ENABLED(USER_PROVIDED_FEATURE_DEFINE)
//          // USER_PROVIDED_FEATURE_DEFINE is enabled
//      #else
//          // USER_PROVIDED_FEATURE_DEFINE is disabled
//      #endif
//
#define WDSC_ENABLED              111-
#define WDSC_DISABLED             333-
...
#define WDSC_IS_ENABLED(x) (333 == 333 * 111 / ((x 0) * (((x 0) == 333 ? 1 : 0) + ((x 0) == 111 ? 1 : 0))))
