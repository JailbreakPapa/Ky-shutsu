...
#if NS_ENABLED(NS_INTEROP_STL_VECTOR)
template <typename T>
inline void nsDynamicArrayBase<T>::operator=(const std::vector<T>& rhs)
{
    nsArrayBase<T, nsDynamicArrayBase<T>>::operator=(
      nsArrayPtr<const T>(rhs.data(), static_cast<nsUInt32>(rhs.size())));
}
#endif
template <typename T> NS_ALWAYS_INLINE T* nsDynamicArrayBase<T>::GetElementsPtr()
{
...
void nsDynamicArray<T, AllocatorWrapper>::operator=(const std::vector<T>& rhs)
{
    nsDynamicArrayBase<T>::operator=(rhs);
}
#endif
