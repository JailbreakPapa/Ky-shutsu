...
{
  /*
  * NOTE(Mikael A.): So, for ApertureUI, we allocate threads based on the requested amount from the user.
  * TODO:
  */
  ...
  nsLog::Dev("CPU core count: {}", iCpuCores);
  nsLog::Dev("Setting worker thread count to {} (short) / {} (long).", uiShortTasks, uiLongTasks);
